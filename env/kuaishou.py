...
def get_distance_mat(
    list_feat, sub_index_list, DATAPATH="environments/KuaishouRec/data"
):
...
def get_similarity_mat(list_feat, DATAPATH="environments/KuaishouRec/data"):
    ...
    if os.path.isfile(similarity_mat_path):
        ...
        print(
            "Compute the similarity matrix (for the first time and will be saved for later usage)"
        )
        for i in tqdm(range(len(df_feat_list)), desc="Computing..."):
            ...
            for j in range(i, len(df_feat_list)):
                similarity_mat[i, j] = len(
                    set(series_feat_list[i]).intersection(set(series_feat_list[j]))
                ) / len(set(series_feat_list[i]).union(set(series_feat_list[j])))
...
class KuaishouEnv(gym.Env):
    ...
    def __init__(
        ...
        self.list_feat_small = list(
            map(lambda x: self.list_feat[x], self.lbe_photo.classes_)
        )

        # smallmat shape: (1411, 3327)

        self.observation_space = spaces.Box(
            low=0, high=len(self.mat) - 1, shape=(1,), dtype=np.int32
        )
        self.action_space = spaces.Box(
            low=0, high=self.mat.shape[1] - 1, shape=(1,), dtype=np.int32
        )
    ...
    def load_mat():
        ...
        df_small = pd.read_csv(
            small_path, header=0, usecols=["user_id", "photo_id", "watch_ratio"]
        )
        ...
        mat = csr_matrix(
            (
                ...
                (
                    lbe_user.transform(df_small["user_id"]),
                    lbe_photo.transform(df_small["photo_id"]),
                ),
            ),
            shape=(df_small["user_id"].nunique(), df_small["photo_id"].nunique()),
        ).toarray()
        ...
        df_feat = pd.DataFrame(
            list_feat, columns=["feat0", "feat1", "feat2", "feat3"], dtype=int
        )
        ...
        df_dist_small = get_distance_mat(
            list_feat, lbe_photo.classes_, DATAPATH=DATAPATH
        )
    ...
    def compute_normed_reward(user_model, lbe_user, lbe_photo, df_photo_env):
        ...
        item_info = item_info[
            ["photo_id", "feat0", "feat1", "feat2", "feat3", "photo_duration"]
        ]
        ...
        for i, user in tqdm(
            enumerate(lbe_user.classes_),
            total=n_user,
            desc="predict all users' rewards on all items",
        ):
            ui = torch.tensor(
                np.concatenate((np.ones((n_item, 1)) * user, item_np), axis=1),
                dtype=torch.float,
                device=user_model.device,
                requires_grad=False,
            )
    ...
    def _determine_whether_to_leave(self, t, action):
        ...
        hist_categories_each = list(
            map(lambda x: self.list_feat_small[x], window_actions)
        )
